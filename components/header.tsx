...
export function Header() {
  ...
  const authUI = !mounted ? null : loading ? (
    <div className="h-9 w-20 animate-pulse rounded-md bg-slate-200"></div>
  ) : user ? (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="flex items-center gap-2">
          ...
          {isPremium && (
            <Badge variant="outline" className="ml-1 hidden bg-blue-50 px-1.5 text-xs text-blue-700 sm:inline-flex">
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        {!isPremium && (
          <DropdownMenuItem onClick={scrollToPricing} className="cursor-pointer">
