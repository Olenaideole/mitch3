        ...
        try {
          ...
          if (!supabase) {
            ...
            if (uploadError) {
              ...
              const {
                data: { publicUrl },
              } = supabase.storage.from("scan-images").getPublicUrl(fileName)
      ...
      <div className="mb-4 text-center text-sm">
        ...
        <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
          <div
            ...
            style={{ width: `${(scansUsed / FREE_SCAN_LIMIT) * 100}%` }}
          ></div>
        </div>
      </div>
    )
  ...
  return (
    <Card className="w-full max-w-md">
      <CardContent className="p-6">
        ...
        {!preview ? (
          <div
            className="flex h-64 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-6 transition-colors hover:border-blue-400 hover:bg-slate-100"
            onDrop={handleDrop}
