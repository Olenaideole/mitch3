...
export default function ResultsPage() {
    ...
    const getGlutenStatusIcon = (status) => {
        switch (status) {
            case "no":
    ...
    const getGlutenStatusBadge = (status) => {
        switch (status) {
            case "no":
            ...
            case "unknown":
            default:
                return (<Badge variant="outline" className="bg-slate-100 text-slate-700">
            Unknown
          </Badge>);
        }
    };
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <Card className="mb-6">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-xl">
                  ...
                  {result.gluten_detected === "no" ? (<Badge className="bg-green-100 text-green-800">Safe</Badge>) : result.gluten_detected === "yes" ? (<Badge className="bg-red-100 text-red-800">Contains Gluten</Badge>) : (<Badge className="bg-slate-100 text-slate-800">Unclear</Badge>)}
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            : `${result.cross_contamination_risk.charAt(0).toUpperCase() +
                result.cross_contamination_risk.slice(1)} Risk`}
                    </Badge>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {result.additives_detected && result.additives_detected.length > 0 ? (result.additives_detected.map((additive, index) => (<Badge key={index} variant="outline" className="bg-slate-100">
                            {additive}
                          </Badge>))) : (<Badge variant="outline" className="bg-green-50 text-green-700">
                          None Detected
                        </Badge>)}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className={result.certification === "yes"
                ...
                : "bg-slate-100"}>
                  ...
                  <div className="mt-4 rounded-lg bg-slate-50 p-3">
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200">
                      <div className="h-full rounded-full bg-green-500" style={{ width: result.community_safe_rating }}></div>
                    </div>
                  </div>
